...
; djm, 27/6/6
;
; Upon finding '2006/' it writes '2006/', then followed by next lot of text until 0dh is found.
...
notes   	DB      cr,lf,'wsd 10. djm, 27/6/6.  For weather station... '
        	DB      cr,lf,'Upon finding 2006/ will write this & rest of line out. Everything else ignored.'
        	DB      cr,lf,' To clean up the raw listing of .DM2 into lines of individual hourly data.'
        	DB      cr,lf,'Used for the .DM2 file',cr,lf
        	DB      cr,lf,'wsd10 <input file> <output file>',cr,lf, 0
dot		db	'.',0
option1		db	0
http		db	'2006/',0
...
@@50:	
	...
	call	readbyte
        cmp     al, '/'
        jne     @@50
;        mov     di, offset option1
;        mov     al, [di]
;        cmp     al, 0                   ; if first time for '2006/' then write out '2006/'
;        jne     @@65
;        mov     di, offset option1
;        mov     al, 1
;        mov     [di], al
        ...
        mov     dx, offset http         ; Address variable with ds:dx
...
@@65:	call	readbyte
	...
	je	@@66
        ...
        int     21h                     ; Call DOS to write to file
	jmp	@@65
@@66:   mov     ah, 40h                 ; DOS Write-file function
        mov     bx, [outFile]           ; Set bx to output file handle
        mov     cx, 2                   ; Specify one byte to write
        mov     dx, offset hitenter     ; Address variable with ds:dx
        int     21h                     ; Call DOS to write to file
        mov     di, offset dot          ; Address text with di
        call    StrWrite                ; Display notes
