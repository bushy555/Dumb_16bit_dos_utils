; Header. v1
;
; deletes <param 2> number of lines from the top,
; then copies rest to <filename>.
...
notes           DB      cr,lf,'Header. v1.  /Bushy, 15-10-2k3.', cr,lf
        	DB      cr,lf,'Deletes x number of lines then dumps the rest to <filename>'
        	DB      cr,lf,'header <in-file> <out-file> <num of lines to kill>'
                db      cr,lf,'example:  header in.txt out.txt 12'
                db      cr,lf,'          will kill the first 12 lines, then dump the rest.'
        	DB      cr,lf,cr,lf
