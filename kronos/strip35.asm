...
@@80:   
        mov     ah, 40h                 ; DOS Write-file function
        mov     bx, [outFile]           ; Set bx to output file handle
        mov     cx, 22                  ; Specify one byte to write
        mov     dx, offset mess1        ; Address variable with ds:dx
        int     21h                     ; Call DOS to write to file
	dec	si
	push	si
	mov	ax, si
	...
	int     21h
        ...
        mov     bx, [outFile]           ; Set bx to output file handle
        mov     cx, 22                   ; Specify one byte to write
        mov     dx, offset mess2       ; Address variable with ds:dx
        int     21h                     ; Call DOS to write to file
	dec	bp
	push	bp
	mov	ax, bp
	...
	int     21h
        ...
        mov     bx, [outFile]           ; Set bx to output file handle
        mov     cx, 22                   ; Specify one byte to write
        mov     dx, offset mess5       ; Address variable with ds:dx
        int     21h                     ; Call DOS to write to file
	pop	bp
	pop	ax
	sub	ax, bp
