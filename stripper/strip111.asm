; Stripper 111
; Copies (reads and writes) first 3x 65536 bytes (196 kb), then ignores the rest of the file.
; For binary files.
;
...
crlf		db	cr,lf,0
notes   	DB      cr,lf,'Stripper39  /djm,  16/1/7', cr,lf
        	DB      cr,lf,'Stripper39  <in-file>  <out-file>'
        	DB      cr,lf,cr,lf,0
...
@@40:   mov     [outFile], ax           ; Save output file handle
	...
	mov	bx, 0FFFFh		; READ 65535 times.
@@_41:	
	call	readbyte		;      1x 65535 times
	call	writebyte
	call	readbyte		;      2x 65535 times
	call	writebyte
	call	readbyte		;      3x 65535 times
	call	writebyte

	loop	@@_41


					; Have now read-from and written-to 3x 63356 bytes.

@@_42:	call	readbyte		; Now we loop forever reading away merrily, until the end of file.
