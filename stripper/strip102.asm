;
;
; last character Stripper
; djm, 18/11/05
;
; Upon finding the last character in the iput file, will kill it.
; STRIP10.EXE <in-file> <out-file>
...
oneByte2         Db     ' '       ; Byte I/O variable
oneByte3         Db      0       ; Byte I/O variable
...
@@40:   mov     [outFile], ax           ; Save output file handle
        ...
        jnc     @@41                    ; Jump if no error detected
        jmp     Errors                  ; Else jump to error handler
@@41:   or      ax, ax                  ; Check for end of input file
        jz      @@80                    ; ax=0=end of file; jump
        mov     di, offset oneByte
	mov	al, [di]
	mov	di, offset oneByte2
	mov	[di], al
@@50:   mov     ah, 3Fh                 ; DOS Read-file function47W Y [Q497U
        mov     bx, [inFile]            ; Set bx to input file handle
        mov     cx, 1                   ; Specify one byte to read
        mov     dx, offset oneByte      ; Address variable with ds:dx
        int     21h                     ; Call DOS to read from file
        jnc     @@42                    ; Jump if no error detected
        jmp     Errors                  ; Else jump to error handler
@@42:   or      ax, ax                  ; Check for end of input file
        jz      @@80                    ; ax=0=end of file; jump
        mov     ah, 40h                 ; DOS Write-file function
        mov     bx, [outFile]           ; Set bx to output file handle
        mov     cx, 1                   ; Specify one byte to write
        mov     dx, offset onebyte2      ; Address variable with ds:dx
