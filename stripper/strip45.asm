; Stripp44  /djm,  5/6/7
;
; dumps first <99> characters at offset 0, kills rest until <EOF>. Writes <EOF>.
;
; STRIP45 in.txt out.txt 5
; STRIP45 in.txt out.txt 34
;
;
;
        IDEAL
...
crlf		db	cr,lf,0
notes   	DB      cr,lf,'Strip45  /djm,  6/6/7', cr,lf
        	DB      cr,lf,'Dumps first <99> characters at offset 0 per line. (1 to 99). Kills the rest.'
        	DB      cr,lf,'Stripp45  <in-file>  <out-file> <99>',cr,lf,cr,lf
		db	'ie, Strip45 in.txt out.txt 5',cr,lf,'    Strip45 in.txt out.txt 34',cr,lf
		db	'(Go easy but. Isn''t fully tested yet... Has probs if <param> is bigger than',cr,lf
		db	'line length of <in.txt> )' 
	       	DB      cr,lf,cr,lf,0
