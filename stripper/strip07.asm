...
; djm, 01/02/05
;
; Upon finding '\END OF DATA' it writes '\END OF DATA '  - (adds a single space after DATA)
;
;
...
notes   	DB      cr,lf,'Stripper. djm, 01/02/05.'
        	DB      cr,lf,'replaces \END OF DATA with \END OF DATA <space> '
        	DB      cr,lf,'strip7 <input file> <output file>',cr,lf, 0
dot		db	'.',0
option1		db	0
http		db	'\END OF DATA ',0
...
@@50:	call	readbyte	; \      "\END OF DATA"<CR>">"
	cmp	al, '\'
	jne	@@501
	call	readbyte	;E
	call	readbyte	;N
	...
	call	readbyte	;A
        ...
        mov     cx, 13                   ; Specify one byte to write
