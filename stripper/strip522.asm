; Strip52.asm    Zero byte file checker.      djm, 26/7/7
;
; strip52 <in-file> <out-file>
;
; Checks directory listing for a ' ' at offset 37 and a '0' at offset 38.
; If both exist, dumps next 12 characters (filename.ext) to output file.
; else, outputs nothing.
;
;
; dir > dir.txt
; Strip52 dir.txt file.log
;
...
crlf		db	cr,lf,0
notes   	DB      cr,lf,'Strip52  /djm,  26/7/7', cr,lf
        	DB      cr,lf,'0 byte file checker.'
        	DB      cr,lf,'Strip52 <in-file> <out-file>'
        	DB      cr,lf,cr,lf,0
...
@@_42:	call	readbyte	; if not, then wont be 0 byte, read rest of line and go to next line.
	cmp	al, 0ah
	jne	@@_42
	jmp	@@_40

@@_43:	call	readbyte	; #37=' ', now check if #38 = '0'.
	cmp	al, '0'
	je	@@_45

@@_44:	call	readbyte	; #38 != '0', so read in rest of line, and go to start of next line.
	cmp	al, 0ah
	jne	@@_44
	jmp	@@_40


@@_45:	call	readbyte	; #38 equalled '0', write out rest of line ie, filename.
	call 	readbyte
	call	writebyte
	call 	readbyte
	call	writebyte
